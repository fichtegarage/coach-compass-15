 ...
 * Wird nur angezeigt wenn gender === 'female'.
 */
...
function getCyclePhase(lastCycleStart: Date, avgCycleLength: number): CyclePhaseInfo {
  ...
  if (dayInCycle <= 4) {
    return {
      phase: 'early_follicular', label: 'Beginn des Zyklus', emoji: '🌱',
      color: 'border-slate-600 bg-slate-800/50', dayInCycle,
  ...
  if (dayInCycle <= ovulationDay - 1) {
    return {
      phase: 'follicular', label: 'Follikelphase', emoji: '⚡',
      color: 'border-orange-500/40 bg-orange-500/10', dayInCycle,
  ...
  if (dayInCycle <= ovulationDay + 1) {
    return {
      phase: 'ovulation', label: 'Ovulationsphase', emoji: '🔥',
      color: 'border-orange-500/60 bg-orange-500/15', dayInCycle,
  ...
  if (dayInCycle <= ovulationDay + 6) {
    return {
      phase: 'early_luteal', label: 'Frühe Lutealphase', emoji: '🌊',
      color: 'border-blue-500/30 bg-blue-500/10', dayInCycle,
      trainingRecommendation: 'Halte die Intensität aufrecht, aber plane etwas mehr Erholung ein. Dein Körper braucht jetzt etwas mehr Regenerationszeit.',
      intensityAdvice: 'Moderat bis hoch',
    };
  }
  if (dayInCycle <= avgCycleLength - 4) {
    return {
      phase: 'luteal', label: 'Lutealphase', emoji: '🔄',
      color: 'border-blue-500/40 bg-blue-500/15', dayInCycle,
  ...
  return {
    phase: 'late_luteal', label: 'Späte Lutealphase', emoji: '🌙',
    color: 'border-slate-500/40 bg-slate-800/50', dayInCycle,
    trainingRecommendation: 'Dein Körper bereitet sich auf den nächsten Zyklus vor. Sanftes Training, Yoga oder Spaziergänge – Bewegung tut gut, ohne zu fordern.',
    intensityAdvice: 'Leicht bis moderat',
  };
}
...
const CycleTracker: React.FC<Props> = ({ clientId }) => {
  ...
  const handleSave = async () => {
    ...
    await supabase
      .from('clients')
      .update({ last_cycle_start_date: lastCycleStart, avg_cycle_length: avgCycleLength } as any)
      .eq('id', clientId);
  ...
  if (!loaded || !isFemale) return null;
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {phaseInfo
              ? <p className="text-xs text-slate-400">Tag {phaseInfo.dayInCycle} · {phaseInfo.intensityAdvice}</p>
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-4 border-t border-white/10 pt-4">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-300">Erster Tag deines aktuellen Zyklus</label>
            <input
              type="date"
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="range" min={21} max={35} value={avgCycleLength}
...
export default CycleTracker;    return {
      phase: 'follicular', label: 'Follikelphase', emoji: '⚡',
      color: 'border-orange-500/40 bg-orange-500/10', dayInCycle,
      trainingRecommendation: 'Dein Körper ist jetzt besonders leistungsfähig. Ideale Zeit für intensive Einheiten, Kraftaufbau und neue Bestleistungen.',
      intensityAdvice: 'Hoch – jetzt Gas geben',
...
export { getCyclePhase };
export type { CyclePhaseInfo };
export default CycleTracker;      label: 'Beginn des Zyklus',
      emoji: '🌱',
