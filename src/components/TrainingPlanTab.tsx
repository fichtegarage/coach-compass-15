...
const ImportDialog: React.FC<ImportDialogProps> = ({ open, onClose, onImported, clientId, trainerId }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await supabase.from('training_plans').update({ is_active: false }).or(`client_id.eq.${clientId},duo_partner_id.eq.${clientId}`).eq('trainer_id', trainerId).eq('is_active', true);
...
const TrainingPlanTab: React.FC<TrainingPlanTabProps> = ({ clientId, clientName }) => {
  ...
  const loadPlans = useCallback(async () => {
    ...
    const { data: plansData } = await supabase.from('training_plans').select('*').or(`client_id.eq.${clientId},duo_partner_id.eq.${clientId}`).eq('trainer_id', user.id).order('created_at', { ascending: false });
  ...
  const handleActivate = async (planId: string) => {
    await supabase.from('training_plans').update({ is_active: false }).or(`client_id.eq.${clientId},duo_partner_id.eq.${clientId}`);
