...
const TrainingPlanTab: React.FC<TrainingPlanTabProps> = ({ clientId, clientName }) => {
  ...
  const handleDelete = async (planId: string, planName: string) => {
    if (!window.confirm(`Plan "${planName}" wirklich löschen? Alle Einheiten und Übungen werden unwiderruflich entfernt.`)) return;
        ...
        <>
          ...
          <div className="flex justify-end pt-2">
            <Button
              ...
              onClick={() => handleDelete(activePlan.id, activePlan.name)}
            >
...
export default TrainingPlanTab;  sessions_per_week: number | null;
  progression_notes: string | null;
  coaching_notes: string | null;
  nutrition_notes: string | null;
  is_active: boolean;
