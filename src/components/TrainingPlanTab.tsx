...
import {
  ...
  CheckCircle, ClipboardPaste, Pencil, Check, X,
} from 'lucide-react';
    ...
    <div className="rounded-xl border border-border overflow-hidden">
      <button className="w-full flex items-center justify-between px-4 py-3 bg-muted/30 hover:bg-muted/50 transition-colors text-left" onClick={() => setOpen(o => !o)}>
        <div className="flex items-center gap-2">
          ...
          {workout.exercises && <span className="text-xs text-muted-foreground">· {workout.exercises.length} Übungen</span>}
        </div>
      ...
      {open && workout.exercises && workout.exercises.length > 0 && (
        <div className="p-3">
          ...
          {workout.notes && <p className="text-xs text-muted-foreground mt-2 italic">{workout.notes}</p>}
        </div>
      )}
      {open && workout.exercises && workout.exercises.length === 0 && (
