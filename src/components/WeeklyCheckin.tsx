  ...
  <div className="flex gap-2 justify-between">
    {[1, 2, 3, 4, 5].map(v => (
      ...
      >
        ...
        <span className={`text-[10px] font-medium leading-tight text-center ${value === v ? 'text-primary' : 'text-slate-400'}`}>
          {labels[v - 1]}
        </span>
      </button>
    ))}
        ...
        <div className="p-6 space-y-5">
          ...
          {step === 'notes' && (
            <textarea
              ...
              rows={3}
              className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary resize-none"
              autoFocus
            />
          )}
          ...
          <div className="flex gap-2">
            ...
            {step !== 'notes' ? (
              <button
                ...
                disabled={!canAdvance}
                className="flex-1 py-3 rounded-xl bg-primary hover:bg-primary/90 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold text-sm transition-colors active:scale-95"
              >
                Weiter →
              </button>
            ) : (
              <button
                ...
                className="flex-1 py-3 rounded-xl bg-primary hover:bg-primary/90 disabled:opacity-40 text-white font-bold text-sm transition-colors active:scale-95 flex items-center justify-center gap-2"
              >
