...
export default BookingPage;Deutschland
...
const BookingPage: React.FC = () => {
  ...
  const remainingDays = useMemo(() => {
    ...
    return differenceInDays(new Date(packageInfo.endDate), new Date());
  }, [packageInfo]);
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="ghost" size="sm"
                        ...
                        onClick={() => {
                          ...
                          const pad = (n: number) => String(n).padStart(2, '0');
                          const fmt = (d: Date) => `${d.getUTCFullYear()}${pad(d.getUTCMonth()+1)}${pad(d.getUTCDate())}T${pad(d.getUTCHours())}${pad(d.getUTCMinutes())}00Z`;
                          const ics = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//Jakob Neumann Personal Training//DE', 'BEGIN:VEVENT', `UID:${s.id}@jakob-neumann.net`, `DTSTART:${fmt(start)}`, `DTEND:${fmt(end)}`, 'SUMMARY:Personal Training – Jakob Neumann', `DESCRIPTION:${sessionTypeLabels[s.session_type] || s.session_type}`, `LOCATION:${s.location || 'Jakob Neumann Personal Training'}`, 'END:VEVENT', 'END:VCALENDAR'].join('\r\n');
                          const blob = new Blob([ics], { type: 'text/calendar;charset=utf-8' });
                          const url = URL.createObjectURL(blob);
