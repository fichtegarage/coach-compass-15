...
const handleLogout = () => {
  ...
  const handleProfilePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const { error: rpcError } = await supabase.rpc('rpc_update_client_profile_photo', { p_token: clientToken, p_photo_url: filePath });
      ...
      {packageInfo?.isDuo && duoPartnerName && (
        <div className="max-w-4xl mx-auto px-4 mt-3 w-full">
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300">
            ...
            Duo-Paket mit {duoPartnerName} · Geteilter Session-Pool
          </div>
        </div>
      )}
                ...
                <div className="flex items-center justify-between text-xs text-slate-500">
                  ...
                  {packageInfo.endDate && (
                    <span className={remainingDays !== null && remainingDays <= 14 ? (remainingDays <= 7 ? 'text-red-400' : 'text-amber-400') : ''}>
                      bis {format(new Date(packageInfo.endDate), 'd. MMM yyyy', { locale: de })}
                    </span>
                  )}
                </div>
                {packageInfo.isDuo && (
                  <p className="text-xs text-blue-400/70 border-t border-slate-700 pt-2">
                    Jede gebuchte Einheit verbraucht einen Credit aus dem gemeinsamen Pool.
                  </p>
                )}
              </div>
            )}
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-white">{format(new Date(slot.start_time), 'HH:mm')} – {format(new Date(slot.end_time), 'HH:mm')} Uhr</p>
                ...
                const renderCard = (b: any, faded: boolean) => (
                  <div key={b.id} className={`rounded-xl border p-3 space-y-2 ${faded ? 'opacity-60 ' : ''}${b.status === 'pending' ? 'bg-amber-500/5 border-amber-500/20' : 'bg-slate-800 border-slate-700'}`}>
                    ...
                    <div className="flex gap-2">
                      ...
                      {b.status === 'pending' && (
                        <button onClick={() => handleCancelRequest(b.id, b.availability_slots?.start_time)} className="text-xs text-red-400 hover:text-red-300 transition-colors">Anfrage zurückziehen</button>
                      )}
                    </div>
                  </div>
                );
